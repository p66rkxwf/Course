...
def calculate_historical_stats(full_df: pd.DataFrame) -> Dict[tuple, float]:
    """
    計算每門課（同名稱+同教師）的歷年平均暴課率。
    """
    if full_df is None or full_df.empty:
        return {}
    
    if '登記人數' not in full_df.columns or '選上人數' not in full_df.columns:
        return {}
    ...
    df['選上人數'] = pd.to_numeric(df['選上人數'], errors='coerce').fillna(0)
    
    # 關鍵修正：確保 Key 正規化（去空白），以便與推薦列表對應
    df['課程名稱'] = df['課程名稱'].fillna('').astype(str).str.strip()
    df['教師姓名'] = df['教師姓名'].fillna('').astype(str).str.strip()

    # 2. 篩選有效資料：登記 > 0 且 選上 > 0
    valid_mask = (df['登記人數'] > 0) & (df['選上人數'] > 0)
    ...
    # 3. 計算單年暴課率
    valid_df['saturation_rate'] = valid_df['登記人數'] / valid_df['選上人數']

    # 4. 分組平均
    avg_rates = valid_df.groupby(['課程名稱', '教師姓名'])['saturation_rate'].mean().to_dict()
...
def get_all_historical_courses_df() -> Optional[pd.DataFrame]:
    ...
    _courses_cache[cache_key] = full_df
    return full_df
...
async def search_courses(q: str, limit: int = 50):
    try:
        ...
        for c in courses:
            ...
            c['historical_saturation'] = stats_map.get((name, teacher), None)
...
async def recommend_courses(request: RecommendRequest):
    try:
        ...
        if request.year is not None and request.semester is not None:
            ...
            # 如果指定特定學期，則需從歷史資料中撈取
            full_history = get_all_historical_courses_df()
            if full_history is not None:
                full_df = full_history # 暫時切換 context
        else:
