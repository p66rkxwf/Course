...
export function renderRecommendResults(courses) {
    ...
    container.innerHTML = courses.map((course, index) => {
        let saturationHtml = '';
        if (course.historical_saturation != null) {
            const satRate = parseFloat(course.historical_saturation);
            const satPercent = (satRate * 100).toFixed(0);
            const badgeClass = satRate >= 1 ? 'bg-danger' : 'bg-success';
            const icon = satRate >= 1 ? '<i class="fas fa-fire me-1"></i>' : '';
            
            saturationHtml = `
                <span class="badge ${badgeClass} border border-white shadow-sm" title="歷年平均暴課率: ${satPercent}% (排除登記=0)">
                    ${icon}暴課率 ${satPercent}%
                </span>`;
        } else {
            saturationHtml = `<span class="badge bg-light text-muted border">無暴課率資料</span>`;
        }
...
export function showCourseDetailModal(course) {
    ...
    if (modalDialog) modalDialog.classList.remove('modal-lg'); 

    const modal = new bootstrap.Modal(modalElement);
    
    // 標題顯示：[代碼] 課程名稱
    document.getElementById('courseDetailTitle').textContent = `${course.課程代碼} ${course.課程名稱 || course.中文課程名稱}`;
    ...
    const syllabusBtn = (syllabusUrl && syllabusUrl.includes('http')) 
        ? `<a href="${syllabusUrl}" target="_blank" class="btn btn-outline-primary btn-sm w-100"><i class="fas fa-external-link-alt me-1"></i>教學大綱</a>` 
        : `<button class="btn btn-outline-secondary btn-sm w-100" disabled>無大綱連結</button>`;

    const selected = parseFloat(course.選上人數 || 0);
    ...
    if (course.historical_saturation != null) {
        const satRate = parseFloat(course.historical_saturation);
        const satPercent = (satRate * 100).toFixed(0);
        const color = satRate >= 1 ? 'text-danger' : 'text-success';
        statsSection = `
            <div class="col-6">
                <div class="p-2 bg-light rounded text-center h-100">
                    <div class="text-muted small mb-1">歷年平均暴課率</div>
                    <div class="fw-bold fs-5 ${color}">${satPercent}%</div>
                </div>
            </div>
        `;
    } else {
        statsSection = `
            <div class="col-6">
                <div class="p-2 bg-light rounded text-center h-100">
                    <div class="text-muted small mb-1">歷年暴課率</div>
                    <div class="text-muted fs-6">-</div>
                </div>
            </div>
        `;
    }
    ...
    body.innerHTML = `
        <div class="container-fluid px-1">
            <div class="d-flex gap-2 mb-3">
                <span class="badge bg-primary">${course.學分} 學分</span>
                <span class="badge bg-secondary">${course.課程性質 || '選修'}</span>
                <span class="badge bg-info text-dark">${course.開課班別 || course['開課班別(代表)'] || '一般'}</span>
            </div>

            <div class="row g-3 mb-3">
                <div class="col-12">
                    <div class="d-flex align-items-center">
                        <div class="avatar-icon bg-primary bg-opacity-10 text-primary rounded-circle me-3 d-flex align-items-center justify-content-center" style="width:40px; height:40px;">
                            <i class="fas fa-chalkboard-teacher"></i>
                        </div>
                        <div>
                            <div class="small text-muted">授課教師</div>
                            <div class="fw-bold">${course.教師姓名 || '未定'}</div>
                        </div>
                    </div>
                </div>
                <div class="col-12">
                    <div class="d-flex align-items-center">
                        <div class="avatar-icon bg-success bg-opacity-10 text-success rounded-circle me-3 d-flex align-items-center justify-content-center" style="width:40px; height:40px;">
                            <i class="far fa-clock"></i>
                        </div>
                        <div>
                            <div class="small text-muted">時間地點</div>
                            <div class="fw-bold">
                                週${course.星期} ${course.起始節次}-${course.結束節次}節
                                <span class="text-muted fw-normal ms-1">@ ${course.上課地點 || '未定'}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row g-2 mb-3">
                ...
                <div class="col-6">
                    <div class="p-2 bg-light rounded text-center h-100">
                        <div class="text-muted small mb-1">本期選課人數</div>
                        <div class="fw-bold fs-5 text-dark">${selected} <span class="fs-6 text-muted fw-normal">/ ${capacity}</span></div>
                    </div>
                </div>
            </div>

            ${course.備註 ? `
                <div class="alert alert-light border border-secondary border-opacity-25 small mb-3">
                    <i class="fas fa-info-circle me-1 text-primary"></i>${course.備註}
                </div>` : ''}
    ...
    if (addBtn) {
        ...
        newBtn.onclick = () => {
            modal.hide();
            if (window.addRecommendedCourse) window.addRecommendedCourse(course);
        };
    }
    modal.show();
}
