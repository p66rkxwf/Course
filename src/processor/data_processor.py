...
`DataProcessor` 會讀取爬蟲產生的原始 CSV，輸出已清理且扁平化的資料集供 API 與分析使用。
此模組為確定性處理，並不執行網路 I/O。
"""
    ...
    def split_teachers_by_dict(text: str, teacher_set: Set[str], max_name_len: int) -> List[str]:
        ...
        while i < n:
            ...
            if matched:
                ...
                i += 1 # 如果沒對到字典，通常會跳過或當作單字處理，這邊簡單跳過
